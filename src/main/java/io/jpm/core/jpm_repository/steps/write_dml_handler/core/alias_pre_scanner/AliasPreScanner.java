...
public class AliasPreScanner {
    ...
    public AliasPreScanner(RepoMetaRegistry repoMetaRegistry) {
        ...
        try {
            ...
            for (int i = 0; i < statements.size(); i++) {
                ...
                if (groupDepth == 0) {
                    ...
                    // 그룹 내부도 재귀 스캔
                    if (isGroupOpen(cmd)) {
                        List<DslStatement> subStmts = GroupExtractorUtil.extract(statements, i);
                        scan(subStmts, ctx);
                    }
                }
    ...
    private void scanJoin(DslStatement stmt, BuildContext ctx) {
        try {
            ...
            EntityMeta meta    = repoMetaRegistry.getEntityMeta(rawClass);
            String actualTable = meta != null
                    ? repoMetaRegistry.getTable(meta.getTableName())
                    : rawClass;
