...
import io.jpm.core.jpm_repository.steps.write_dml_handler.core.sql_node_parser.node.*;
import io.jpm.core.jpm_repository.steps.write_dml_handler.utils.GroupExtractorUtil;
...
public class BuildSqlNodesStep implements Step<SqlNodeParserContext> {
    ...
    private final SqlMapBinder  binder;
        ...
        for (int i = 0; i < statements.size(); i++) {
            ...
            switch (cmd) {
                ...
                case "fromGroup": {
                    ...
                    i += sub.size() + 1;
                    ctx.addNode(new FromSubQueryNode(stmt, sub, entityMeta, binder));
                    break;
                }
                ...
                case "leftJoinGroup": {
                    List<DslStatement> sub = GroupExtractorUtil.extract(statements, i);
                    i += sub.size() + 1;
                    ctx.addNode(new JoinGroupNode(cmd, args, sub, entityMeta, binder, repoMetaRegistry));
                    break;
                }
                ...
                case "whereNotExistsGroup": {
                    ...
                    where.addCondition(new ExistsNode(cmd, sub, entityMeta, repoMetaRegistry, binder));
                    break;
                }
