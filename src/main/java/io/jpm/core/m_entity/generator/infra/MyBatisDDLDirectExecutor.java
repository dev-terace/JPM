package io.jpm.core.m_entity.generator.infra; // 패키지는 상황에 맞게 조정하세요
            ...
            try {
                ...
                if (Files.exists(path)) {
                    try (InputStream is = Files.newInputStream(path)) {
                        props.load(is);
                        LogPrinter.info("✅ application.properties 로드 성공: " + path.toAbsolutePath());
                    }
                } else {
                    LogPrinter.info("ℹ️ 설정 파일이 존재하지 않습니다. (검색 경로: " + path.toAbsolutePath() + ")");
                }
            } catch (IOException e) {
                LogPrinter.error("파일 로드 중 오류: " + e.getMessage());
            }
        } else {
            LogPrinter.info("⚠️ projectDir 옵션이 없습니다. 기본 옵션값만 사용합니다.");
        }
    ...
    private void runMyBatisUpdate(String sql, Map<String, String> config) throws Exception {
        try {
            ...
            try (SqlSession session = factory.openSession()) {
                ...
                session.commit();
                LogPrinter.info("✅ [MyBatis] DDL 실행 성공!");
            }
        } catch (Exception e) {
            LogPrinter.error("❌ [MyBatis] DDL 실행 실패: " + e.getMessage());
            throw e;
        }
    }
