...
public class MqRepoParserV2 {
    ...
    private static void inlineSegmentMethodTree(ProcessingEnvironment env, Trees trees,
        ...
        String segmentTypeName = entityMetaRegistry.getSegmentPath(repoClassName, fieldVarName);// 클래스 풀네임 반환한다고 가정
    ...
    private static List<String> extractTokensTree(MethodInvocationTree call, Map<String, String> argContext, MethodMeta methodMeta) {
        try {
            ...
            ArgValidatorUtil argValidatorUtil = new ArgValidatorUtil();
            for (int i = 0; i < arguments.size(); i++) {
                ...
                argValidatorUtil.saveFirstArgInfoIfMatched(column, command, i);
                argValidatorUtil.validateArgIfMatched(i, lastArgToString(arg));
