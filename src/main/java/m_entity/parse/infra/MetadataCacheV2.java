...
import utils.MFieldParserV2; // 🚀 V2: Tree API 유틸 사용
...
public class MetadataCacheV2 {
     ...
     */
    public static void saveMetadataCache(TypeElement element, List<List<MFieldParserV2.Pair>> rawData) {
     ...
     */
    private static PairResult processFieldsAndFindPkAndFk(TypeElement element, List<List<MFieldParserV2.Pair>> rawData) {
        String pkColumnName = "id"; // 기본값 (PK를 못 찾을 경우 대비)
        List<MField> variables = new ArrayList<>();

        for (List<MFieldParserV2.Pair> fieldRawData : rawData) {
    ...
    private static String extractFkFieldName(List<MFieldParserV2.Pair> fieldRawData) {

        for (MFieldParserV2.Pair pair : fieldRawData) {
