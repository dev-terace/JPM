package jpm_repository.parse.domain.policy.sql_node_parser;

import exception.CustomProcessorException;
import jpm_repository.parse.domain.policy.GroupExtractor;

import jpm_repository.parse.domain.policy.sql_node_parser.node.*;
...
public class SqlNodeParser {
    ...
    public SqlNodeParser(RepoMetaRegistry repoMetaRegistry, ArgResolver argResolver) {
        ...
        try {
            ...
            for (int j = 0; j < subStatements.size(); j++) {
                ...
                if (GroupExtractor.isGroupOpen(s.getCommand())) {
                    ...
                    if (args.size() >= 3) {
                        group.add(new ConditionNode(logic, args.get(0), args.get(1), args.get(2), s.getCommand()));
                    }
                }
            }
            return group;
    ...
    private ConditionNode buildCondition(String logic, DslStatement stmt,
        ...
        return new ConditionNode(
                ...
                stmt.getArgs().get(0),
                args.get(1),
                resolveSqlValue(stmt.getArgs().get(0), rawValue, ctx), stmt.getCommand()
        );
    }
